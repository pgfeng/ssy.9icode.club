 ...
 * User: taoqili and PGF
 * Date: 14-02-08
 * Time: 上午11: 32
 * UEditor编辑器通用上传类结合GFPHP修改版
 */
class upload
{
     ...
     */
    public function uploadFile($fileField, $config, $base64 = false)
    {
    ...
    private function getFolder()
    {
        ...
        $allPath = __ROOT__ . dirname(SCRIPT_PATH) . $pathStr;
        if (strrchr($allPath, "/") != "/") {
            $allPath .= "/";
        }
        $allPath .= date("Ymd");
        if (!file_exists($allPath)) {
            if (!mkdir($allPath, 0700, true)) {
                return false;
            }
        }
        $this->folderName = date("Ymd");
        return $allPath;
    }
    ...
    public function getFileInfo()
    {
        return array(
            ...
            "name" => $this->fileName,
            //图片上传后的url可访问路径
            "url" => dirname(SCRIPT_PATH) . '/' . $this->config["savePath"] . '/' . $this->folderName . '/' . $this->fileName,
            //基于应用位置的路径
            "savePath" => $this->config["savePath"] . '/' . $this->folderName . '/' . $this->fileName,
            "size" => $this->fileSize,
            "type" => $this->fileType,
