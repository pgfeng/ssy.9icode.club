...
<script type="text/javascript">
    BUI.use(['bui/grid', 'bui/data'], function (Grid, Data) {
        ...
        //默认的数据
            data =<?php echo module('admin')->field_list($module['modelid']);?>,
        ...
        function addFunction() {
            window.location.href='<?php echo url('Module/manage/field/add');?>?mid=<?php echo $module['modelid']; ?>';
        }
        ...
        editing.on('accept', function (editing) {
            ...
            {
                var url = '<?php echo url('Module/manage/field/edit/');?>' + '?' + new Date().getTime();
                ...
                buttons : [
                    {
                        ...
                        handler : function(){
                            ...
                            var url = '<?php echo url('Module/manage/field/del');?>?mid=<?php echo $_GET['mid'];?>';
                            $.post(url,{id:$id},function(data){
                                data=JSON.parse(data);
                                if (data.status == 'ok') {
                                    //data.status = 'success';
                                    //BUI.Message.Alert(data['msg'], data.status);
                                    location.href = '<?php echo url('Module/manage/field');?>?mid=<?php echo $_GET['mid'];?>';
                                } else {
                                    data.status = 'error';
                                    BUI.Message.Alert(data['msg'], data.status);
                                }
        ...
        grid.on('cellclick', function (ev) {
            ...
            if (target.hasClass('btn-manage')) {
                ...
                window.location.href='<?php echo url('Module/manage/field/edit/');?>/?mid=<?php echo $module['modelid']; ?>&fid='+record.id;
            }
            /**
