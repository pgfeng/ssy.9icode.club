...
<?php $table=array();?>
<?php if(is_array($admin)) foreach($admin AS $v) { ?>
<?php 
    $data['id']=$v['roleid'];
    ...
    BUI.use(['bui/grid', 'bui/data'], function (Grid, Data) {
        ...
        <?php echo json_encode($table);?>,
            store = new Data.Store({
                ...
                enumObj =<?php echo isset($roledata)?$roledata:$this->var['roledata'];?>,
        columns = [
        ...
        editing.on('accept', function (editing) {
            ...
            if (record.userid > 0) {
                var url = '<?php echo url('Index/admin_manage/edit');?>' + '?' + new Date().getTime();
                $.post(url, {
                ...
                var url = '<?php echo url('Index/admin_manage/add');?>' + '?' + new Date().getTime();
                $.post(url, {
                    userid: record.userid,
                    roleid: record.roleid,
                    username: record.username,
