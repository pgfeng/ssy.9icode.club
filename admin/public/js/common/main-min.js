define("common/main", ["bui/menu", "bui/tab"], function (l) {
    ...
    function i(C, F, I, G, B) {
        var J = this, D = new h.SideMenu(I), E = new m.NavTab(F), K = $(I.render), A = K.next("." + t + "-con"), H = K.parents("." + f);
        if (A) {
            A.on("click", function () {
                H.toggleClass(u)
            });
    ...
    function d() {
        ...
        return {moduleId: D, pageId: A, search: C}
    }
    ...
    BUI.augment(x, {
        openPage: function (P) {
            var K = this, F = P.moduleId || K._getCurrentModuleId(), D = P.id, M = P.title || "\u65b0\u7684\u6807\u7b7e\u9875", C = P.href, O = P.isClose, E = P.closeable, B = P.reload, Q = P.search;
            var H = K._getModule(F);
            if (H) {
                var I = H.tab, G = H.menu, A = G.getItem(D), L = I.getActivedItem(), N = L ? L.get("id") : null, J = K._getModuleIndex(F);
                ...
                if (A) {
                    ...
                    I.addTab({id: D, title: M, href: C, sourceId: N, closeable: E}, B)
                }
                if (O) {
                    L.close()
                }
            ...
            if (C) {
                var D = C.tab, E = H ? D.getItemById(H) : D.getActivedItem();
                if (E && E[G]) {
                    E[G].apply(E, B)
                }
            }
        }, _createModule: function (F) {
            var A = this, D = A._getModuleConfig(F), C = A.get("modules");
            if (!D) {
                return null
            }
            var F = D.id, B = "#J_" + F + "Tab", E = "#J_" + F + "Tree";
            module = new i(F, {render: B, height: a() - 5}, {
                render: E,
                items: D.menu,
                height: a() - 5
            ...
            var J = this, D = J.get("navItems"), C = J.get("hideItmes");
            if (D.length === 0) {
                return
            }
            $('<div class="nav-item-mask"></div>').appendTo($(D));
            var H = D.length, L = BUI.viewportWidth(), I = j, G = I * H, A = 0;
            if (G <= L) {
                return
            }
            $.each(D, function (M, N) {
            ...
            var C = this, B = C.get("hideList"), A = C.get("hideItmes");
            if (B) {
                return
            }
            var E = '<ul class="dl-hide-list ks-hidden"></ul>', D = $(E).appendTo("body");
            B = D;
            $.each(A, function (F, G) {
                $(G).appendTo(B)
            });
            ...
            var A = this, C = A.get("modulesConfig"), B = A.get("navContent");
            B.children().remove();
            $.each(C, function (E, F) {
                var G = F.id, D = ['<li class="dl-tab-item ks-hidden"><div class="dl-second-nav"><div class="dl-second-tree" id="J_', G, 'Tree"></div><div class="', t, '-con"><div class="', t, '"></div></div></div><div class="dl-inner-tab" id="J_', G, 'Tab"></div></li>'].join("");
                new $(D).appendTo(B)
            })
        }, _initDom: function () {
            var A = this;
            ...
            var B = this, A = B.get("hideList"), C = B.get("navList");
            ...
            var A = this, E = d();
            if (E) {
                var C = E.pageId, F = E.search, D = A._getModuleIndex(E.moduleId);
                A._setModuleSelected(D);
                A._setPageSelected(D, C, true, F)
            } else {
                var B = A.get("currentModelIndex"), G = A._getModuleId(B);
                if (B == null) {
            ...
            var A = this, E = A._getCurrentModuleId(), C = A._getModule(E), B = "";
            if (C) {
            ...
            var I = this, E = I.get("navItems"), C = I.get("navTabs"), H = I.get("currentModelIndex");
            if (H !== G) {
                var A = I._getModuleId(G), B = null, D = I.get("lastShowItem"), J = true;
            ...
            var I = this, C = I._getModuleId(F) || F, D = I._getModule(C);
            if (D && G) {
                ...
                if (K && K.get("id") === G) {
                    B = K.get("href");
                    B = L ? (b(B, L)) : B;
                    D.tab.addTab({id: K.get("id"), title: K.get("text"), closeable: K.get("closeable"), href: B}, !!H)
                } else {
                    if (G) {
                        var J = G.replace("-", "/");
                        if (J.indexOf("/") === -1) {
            ...
            var C = this, B = C.get("hideList"), A = C.get("hideItmes"), F = null, D = null, H = null, G = null;
